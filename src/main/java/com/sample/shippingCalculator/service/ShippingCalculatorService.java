...
public class ShippingCalculatorService {
	...
	public ResponseEntity<ItemDetail> calculate(ItemDetail itemDetail) {
		...
		if (hasInvalidValue(itemDetail)) {
			throw new ResponseStatusException(
			          HttpStatus.BAD_REQUEST, ValueConditionConstants.invalidValueMsg);
		} else {
	...
	public ParcelType getParcelType(ItemDetail itemDetail) {
		...
		if (itemDetail.getWeight() > ValueConditionConstants.maxWeight) {
			...
			throw new ResponseStatusException(
			          HttpStatus.NOT_FOUND, ValueConditionConstants.typeNotFound);
		}	
	}
	
	private boolean hasInvalidValue(ItemDetail itemDetail) {
		if (itemDetail.getWeight() == 0 || itemDetail.getHeight() == 0 ||
				itemDetail.getWidth() == 0 || itemDetail.getLength() == 0) {
			return true;
		}
		return false;
	}
